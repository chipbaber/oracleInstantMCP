# Instuctions
This markdown will walk you through how to create your own MCP server in python with 3 tools that call an Oracle 26ai database running on OCI. The tools will showcase how to query the table comments so the LLM understands the data, will show how to query and return a limited set of data from a single table exposed as a tool. Then show how to create and call a tool that executes a pl/sql procedure and returns a result. 
...
-  Paste in the statements below to create the required objects for the sample demo.
```
...
- We are going to slowly build and explain to you the code in 26ai_fastmcp.py in the video.
